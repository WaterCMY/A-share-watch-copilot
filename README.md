...
- **持仓管理体系**：`positions.json` schema、意图标签（长持 / 中线 / 卫星仓 / 逢低补仓…）、止损止盈设定法
- **8 个自动化任务模板**：盘前摘要 / 竞价速览 / 盘中止损监控 / 午间总结 / 科创50回踩补仓 / 盘后总结 / 断线哨兵（含 rrule + prompt）
- **数据源选型与踩坑经验**：westock-mcp 断连兜底、market_overview 滞后、批量 kline 单股报错等
- **报告模板**：盘前 / 盘后 / 午间 / 交易计划 / 持仓建议
...
5. **设置原生提醒**：在 westock 设为 each 标的的到价提醒（low=止损 / high=止盈）作为实时兜底。
