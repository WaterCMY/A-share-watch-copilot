...
- 🖥️ **本地工作台（可选）**：单页看盘 HTML + 本地代理，五个 Tab（个人持仓 / 大盘全景 / 养鸡场 / 整体交易记录 / 📈 资金曲线），支持实时行情、K线/分时/30分/1时弹窗、手动录入/卖出减仓/调整成本、一键同步回持仓档案
- 🐔 **养鸡场（场外基金）**：跟踪场外开放式基金每日净值与持有收益，支持按销售平台分组筛选与平台总览（分平台 + 合计）
- 📱 **手机端适配**：工作台窄屏自动切换卡片式表格 + K线自适应，手机浏览器读感友好；同 Wi‑Fi 下访问本地服务即可
- 🕐 **K线周期扩展**：弹窗新增 30分 / 1时 线，分钟线显示时分标签
- 📈 **资金曲线（第 5 Tab）**：`account_curve.py` 按各标的 trades 还原每日持股数与**移动平均成本池**，回补每日市值（A股东财历史K线 + 场外基金净值），绘制账户日K面积图 + 日涨跌红绿柱 + 最大回撤 / 新高日统计；盘后自动化每日刷新。**注意**：该 Tab 依赖 `portfolio_history.json`，需先 `python account_curve.py` 生成（示例仓库不含此文件）；单只标的行情抓取失败时会退回快照价兜底，并在 Tab 顶部红字标注缺口，对应市值可能低估。
- 🧩 **组合集中度诊断**：科技 / 半导体等单主题占比超阈值时弹预警，防范过度集中风险。
- 🔌 **前端抗掉线**：实时行情优先浏览器直连腾讯 `qt.gtimg.cn`（CORS + GBK 解码），本地代理仅作兜底，彻底绕开受限代理导致的前端"卡死"。
- 🗂️ **单一数据源**：持仓自 `positions.json`、基金自 `funds.json` 加载，前端内联数组仅作兜底，改动只需在一处。
- 🌊 **北向资金快照**：`northbound_snapshot.py` 抓取东方财富沪深港通净买入（仅沪股通口径，深股通常缺失须注明），已接入盘后报告。
- 🔒 人在回路：只辅助决策，不代你下单
...
- **资金曲线历史深度依赖股票 K 线**：东方财富基金净值接口（`lsjz`）近期对非登录请求仅返回最近约 20 条净值，故场外基金无法独立回填长周期历史；曲线时间轴长度主要由 A 股/ETF 历史 K 线（可回补约 90 个交易日）决定。若股票行情抓取全失败，曲线将收缩至约 21 天。
- **缺口兜底非实时**：个别标的行情抓取失败时，`account_curve.py` 会退回该标的最新快照价（持仓 `current_price` / 基金 `snap`）作兜底，曲线 Tab 顶部会以红字标注缺口，对应市值可能低估。
- **成本池近似**：基线持仓按当前均价初始化成本池并逐笔滚动，与 `positions.json` 中 `cost_price` 的最终均值可能存在 <1% 偏差（建仓日不明所致），早期绝对水平为近似值，但每日涨跌幅度准确。
- **北向资金仅沪股通口径**：深股通接口常缺失，盘后报告均注明"仅沪股通"。
...
├── README.md                      # 本文件
├── LICENSE                        # MIT + 投资风险提示
├── account_curve.py               # 账户每日资金曲线生成器（盘后自动化调用，输出 portfolio_history.json）
├── northbound_snapshot.py         # 北向 / 沪深港通资金流向快照（盘后报告调用）
