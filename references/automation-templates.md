...
② 读取 positions.json，列持仓速览（代码/成本/现价/盈亏/距支撑压力位空间）；
③ 标注今日需重点关注的支撑/压力位预警标的；
...
- **prompt**：
```
盘中监控（每10分钟）：读取 positions.json，对每标的计算现价距支撑/压力位比例。
仅当满足以下任一条件才推送：① 逼近支撑≤2% 或 跌破；② 逼近压力≤3% 或 突破；
③ 用户标记短期不卖的标的，触及原压力位仅提示不催卖；④ 长持标的仅报支撑触发。
其余情况静默。推送用中文，涨红跌绿，列出代码/现价/触发类型/参考动作。
```
...
## 5. 特定标的价格区间监控（每 10 分钟）

- **rrule**：同 #3（`BYHOUR=9,10,11,13,14;BYMINUTE=0,10,20,30,40,50`）
- **connectorIds**：`["westock-mcp"]`
- **prompt**：
```
监控用户指定标的的价格区间：① 价格进入预设区间 → 推送 🔵 关注；
② 价格跌破区间下沿 → 推送 ⚠️ 深跌预警；③ 区间上方 → 静默。中文，简洁。
```
...
- 搜不到（返回 No matching tools） → 判定 westock-mcp 已断连，立即推送 ⚠️ 提醒，
  内容含：受影响自动化清单（价格监控/区间监控/午间/盘后）+ 重连指引（连接器页点"信任"）。
```

## 8. （可选）信息整理模板 / 次日参考

- 非固定时刻，由用户在盘后触发；或设一次性 `scheduleType=once`。
- 结构见 `report-templates.md` 的"信息整理模板"。
