...
- 能力：实时行情（60s 刷新）、持仓表格（表头排序）、K线/分时/周K弹窗（MA5/10/20/120/250 + 量能）、手动录入仓位、卖出减仓（核算已实现盈亏）、调整成本价/股数、一键同步回 `positions.json`；新增「📈 资金曲线」Tab（`account_curve.py` 生成）、组合集中度诊断、前端行情直连腾讯 CORS 抗掉线、持仓/基金单一数据源。
