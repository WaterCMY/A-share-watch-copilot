...
- 五个 Tab：**📊 个人持仓**（场内）/ **🌐 大盘全景**（指数、市场宽度）/ **🐔 养鸡场**（场外基金）/ **🧾 整体交易记录**（合并持仓与基金的每笔流水总账）/ **📈 资金曲线**（账户每日市值日K）。
- 能力：实时行情（60s 刷新）、持仓表格（表头排序）、K线/分时/周K弹窗（MA5/10/20/120/250 + 量能）、手动录入仓位、卖出减仓（核算已实现盈亏）、调整成本价/股数、一键同步回 `positions.json`；新增「📈 资金曲线」Tab（`account_curve.py` 生成）、组合集中度诊断、前端行情直连腾讯 CORS 抗掉线、持仓/基金单一数据源；顶栏「总投资金额」输入（localStorage 持久化）、单标的仓位占比列、🗃️ 仓库管理提醒框、总持仓醒目卡（股票/基金拆分 + 各基金明细 + 当前总仓位%）。
